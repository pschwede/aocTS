...
function run(memory: Array<number>, debug?: boolean): Array<number>{
...
function test(): boolean {
	return assertEqual([1,2,3,4], parseProgram(`1,2,3,4`))
		&& assertEqual(parseProgram(`3500,9,10,70,2,3,11,0,99,30,40,50`), run(parseProgram(`1,9,10,3,2,3,11,0,99,30,40,50`)))
		&& assertEqual(parseProgram(`2,0,0,0,99`), run(parseProgram(`1,0,0,0,99`)))
		&& assertEqual(parseProgram(`2,3,0,6,99`), run(parseProgram(`2,3,0,3,99`)))
		&& assertEqual(parseProgram(`2,4,4,5,99,9801`), run(parseProgram(`2,4,4,5,99,0`)))
		&& assertEqual(parseProgram(`30,1,1,4,2,5,6,0,99`), run(parseProgram(`1,1,1,4,99,5,6,0,99`)));
}
