...
class WireMap {
    ...
    public static fromString(wire: string): WireMap {
        let x: number = 0;
        let y: number = 0;
...
function shortestDistanceFromOriginOfOverlay(wires: string): number {
    let candidates: [number, number][] = positionsOfOverlays(overlayMaps(wires.split("\n").map(WireMap.fromString)));
    let distances: number[] = candidates.map(manhattanDistance).filter((x) => x > 0);
...
function main(): number {
    console.log(shortestDistanceFromOriginOfOverlay(wires));
